...
//-----------------------------------------------------------------------------
/**
 * The resource class. Allows to be collected as a garbage if not use for some
 * time or ticks
 *
 * @class CacheEntry
 * @constructor
 ...
 * @param {string} item - Bitmap, HTML5Audio, WebAudio - whatever you want to
 *     store in the cache
 */
...
/**
 * Performs a block transfer, using assumption that original image was not
 * modified (no hue)
 *
 * @method blt
 * @param {Bitmap} source The bitmap to draw
 ...
 * @param {Boolean} vertical Wether the gradient should be draw as vertical or
 *     not
 */
...
Input.update = function () {
  ...
  for (var name in this.currentState_) {
...
Input._updateGamepadState = function (gamepad) {
  ...
  for (var j = 0; j < newState.length; j++) {
    if (newState[j] !== lastState[j]) {
      ...
      if (buttonName) {
        this.currentState_[buttonName] = newState[j];
      }
    }
  }
